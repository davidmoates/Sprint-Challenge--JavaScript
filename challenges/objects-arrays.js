...
let universities = graduates.map((unABC) => unABC.university);
...
const contactInfo = graduates.map((con) => `${con.first_name} ${con.email}`);
console.log(contactInfo);

/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called uni that contains them all. Log the result. */
const uni = graduates.filter((start) => start.university.includes("Uni"));
console.log(uni);
...
*/
const animalNames = [];
const fullName = zooAnimals.forEach((name) =>
...
const lowerCase = zooAnimals.map((lowCase) => lowCase.animal_name.toLowerCase());
...
*/
const populationTotal = zooAnimals.reduce((total, totalPop) => total += totalPop.population, 0);
